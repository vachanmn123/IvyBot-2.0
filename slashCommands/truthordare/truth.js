...
module.exports = {
  ...
  description: "Gives a Truth question", //the command description for Slash Command Overview
  cooldown: 1,
  memberpermissions: [], 
  requiredroles: [], 
  alloweduserids: [], 
  options: [
        ...
        {"User": { name: "user", description: "The reciever who will get the truth question", required: false }},
  ],
  run: async (client, interaction) => {
    try{
        const res = await axios.get(`https://api.truthordarebot.xyz/v1/truth?rating=${interaction.options.get("rating")? interaction.options.get("rating").value : "pg"}`);
